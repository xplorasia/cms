...
<script>
  export default {
    ...
    methods: {
      getList: function () {
        this.loading = true
        this.$http.get(process.env.API_URL + '/currencies').then(response => {
      ...
      editThis: function (item) {
        this.$router.push('/currency/form?id=' + item._id)
      },
      deleteThis: function (item) {
        this.loading = true
        this.$http.delete(process.env.API_URL + '/api/currencies/' + item._id).then(response => {
          this.getList()
        }, response => {
          // error callback
        })
